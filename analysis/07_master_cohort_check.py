...
def cohorts(rows):
    ...
    bu = sum(1 for r in rows if r["_inc"] and r["_app"] and r["_valid"] and r["_inw"] and r["_resFin"]==UNRES)
    buR = sum(1 for r in rows if r["_inc"] and r["_app"] and r["_valid"] and r["_inw"] and r["_resL"]==UNRES)
    print(f"        of which Cochrane catchment          {bc:9,}")
    print(f"      Cochrane placement, residency UNRESOLVED {bu:9,}   (own category; never A or B; after registry AND Strata. Registry-only: {buR:,})")
